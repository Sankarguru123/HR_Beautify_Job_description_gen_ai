...
# Backend API URL
# BACKEND_URL = "http://127.0.0.1:8000" # local url
BACKEND_URL = "http://backend:8000"   # only purpose  docker  backend url
# job_names_history = {}
...
#Deploy Button and Developer option  hide
...
def logout():
    ...
    st.experimental_set_query_params(page='',job_id='',token='')
...
def main():
    ...
    if "expiration_time" not in st.session_state:
        st.session_state["expiration_time"] = datetime.now() + timedelta(minutes=30) if token_from_url else None
